...
其他说明：无
**********************************************************/
void LCD_SGP18T_init(void)
{ 	
  gpio_init(PORTE,0,GPO,0);//CS
  gpio_init(PORTE,1,GPO,0);//SCK
  gpio_init(PORTE,2,GPO,0);//SDI
  gpio_init(PORTE,3,GPO,0);//A0
  gpio_init(PORTE,4,GPO,0);//RST
  
  RESET = 0;//ILI9163_RESET=0;                                
  DLY_us(5);
  RESET = 1;//ILI9163_RESET=1;                     
  DLY_us(10); 
...
void LCD_write_command(u8 cmd)
{
	...
	A0 = 0;// A0=0 ILI9163_A0=0;	 						//A0=0发送命令
	CS = 0;// CS=0 ILI9163_CS=0;
	for(i=0;i<8;i++)
	{
		SCK = 0;// SCK=0  ILI9163_SCK=0;
		 if(cmd&0x80){SDI = 1;}// SDI=1
                        else{SDI = 0;}// SDI=0 
		SCK = 1;// SCK=1  ILI9163_SCK = 1;
		cmd = (cmd<<1);
	}
       CS = 1;// CS=1  ILI9163_CS=1;
}

/*****************DRIVE IC寄存器写数据 8080 SERIES******************************/
void LCD_write_para8(u8 dat)
{  
	u8 i;

	A0 = 1;// A0=1  ILI9163_A0=1;			   				//A0=1发送数据
	CS = 0;// CS=0  ILI9163_CS=0;				
	for(i=0;i<8;i++)
	{
		SCK = 0;// SCK=0  ILI9163_SCK=0; 		
                
                if(dat&0x80){SDI = 1;}// SDI=1
                        else{SDI = 0;}// SDI=0               
...
void LCD_write_para16(u16 dat)
{  		 
	u8 i,buf;
	A0 = 1;// A0=1  ILI9163_A0=1;
	CS = 0;// CS=0  ILI9163_CS=0;				
...
其他说明：无
**********************************************************/
void LCD_SetPos(u8 xs,u8 ys,u8 xe,u8 ye)
{
...
其他说明：无
**********************************************************/
void LCD_display_full(u16 color)
{
  	unsigned int i,j;
...
/**********************************************************
函数名称：LCD_draw_part()
入口参数：起始、终止横坐标(0-127)，纵坐标(0-159),显示颜色u16
出口参数：无
时间：2012-09-09
功能说明：填充矩形区域
其他说明：0<=xs<xe<=127
...
入口参数：圆心横坐标(0-127)，纵坐标(0-159),半径(0-128),显示颜色u16
出口参数：无
时间：2012-09-09
功能说明：画圆形边框(仅支持屏幕内画圆)
其他说明：0<=x<=127
...
/**********************************************************
函数名称：LCD_draw_dot()
入口参数：起始横坐标(0-127)，纵坐标(0-159),显示颜色u16
出口参数：无
时间：2012-09-09
功能说明：画点
其他说明：0<=x<=127
		  0<=y<=159
**********************************************************/
void LCD_draw_dot(u8 x,u8 y,u16 color_dat)
{
	LCD_SetPos(x,y,x,y);
	LCD_write_para16(color_dat);
}
...
/*
void LCD_P14x16Str(unsigned char x,unsigned char y,u8 *s_dat,u16 word_color,u16 back_color)
{
	unsigned char wm=0,ii = 0,i,j;
	unsigned int adder=1; 
