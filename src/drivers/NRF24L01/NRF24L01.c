...
#define NRF_CE_HIGH()	    PTA4_OUT=1
#define NRF_CE_LOW()	    PTA4_OUT=0			  //CE置低
#define NRF_PCSN_LOW()      PTA5_OUT=0
#define NRF_PCSN_HIGH()     PTA5_OUT=1
#define NRF_Read_IRQ()	    PTA19_IN
...
void NRF_Init(void)
{
    ...
    gpio_init(PORTA,4, GPO,LOW);           //初始化CE，默认进入待机模式
    //gpio_init(PORTE,27, GPI,LOW);           //初始化IRQ管脚为输入 
    gpio_init(PORTA,5, GPO,HIGH);          //初始化PCSN管脚为输出，低电平选中  
    
#if IS_USE_ISR
    exti_init(PORTA,19, falling_up);        //初始化IRQ管脚为 :下降沿触发，内部上拉
#else
    ...
    NRF_WriteReg(NRF_WRITE_REG+RF_CH,CHANNEL);               //设置RF通道为CHANNEL
...
void Send_Image(void)
{
  ...
  for(i = 0; i < 128; i++)
  {
    for(j = 0; j < 128; j++)
    {
      txbuf[0] = Camera_Data[i][j]/256;
      txbuf[1] = Camera_Data[i][j]%256;
      
      NRF_Tx_Dat(txbuf,2);	//把摄像头RGB565数据发送出去
    }
  } 
}